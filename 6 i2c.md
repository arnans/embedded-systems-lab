...
### ปฏิบัติการ
  ...
  * Lab 1 — ฝึกใช้ PCF8574 I/O Expander ผ่าน Library สำเร็จรูป
  * Lab 2 — ฝึกควบคุม PCF8574 ด้วยคำสั่ง I2C
  * Lab 3 — สร้างนาฬิกา โดยใช้ DS1307 Realtime Clock และ 16x2 Display และใช้ Library สำเร็จรูป
  * Lab 4 เขียนโปรแกรมอ่านค่าจาก DS1307 โดยใช้คำสั่ง i2c โดยตรง
  * Lab 5 — Reverse Engineer โมดูล LCD Display เพื่อควบคุมบางความสามารถผ่าน i2c โดยตรง
...
Adafruit_I2CDevice i2c_dev = Adafruit_I2CDevice(PCF8574_Address);  
uint8_t buffer[32];  
unit8_t bitConfig = 0b00000000; // P0-P7 config bits  
...
void loop() {  
  ...
  bitConfig |= 0b10000000;  // or you can write bitConfig |= (1<<7)  
  buffer[0]=bitConfig;     
  i2c_dev.write(buffer,1);  // turn off LED  
  delay(1000);  
  
  // set bit 7 to drain (logic low)  
  bitConfig &= 0b01111111;  // or you can write bitConfig &= ~(1<<7)  
  buffer[0]=bitConfig;     
...
while (1) {  
  delay(100);   // this delay is optional  
  
  // read the pin state from PCF8574  
  i2c_dev.read(buffer,1);  
  P0_state = buffer[0] & 0b00000001;  // read the state of P0  
...
### Lab 3 — สร้างนาฬิกา โดยใช้ DS1307 Realtime Clock และ 16x2 Display และใช้
Library สำเร็จรูป

ปฏิบัติการนี้จะเพิ่มอุปกรณ์ลงไปในด I2C Bus อีก 2 ชิ้น คือ realtime clock และ
display
...
### ให้ทำตามขั้นตอนต่อไปนี้
  ...
  1. ศึกษาการเชื่อมต่อวงจร และโปรแกรมตัวอย่างจ่าง Simulator ต่อไปนี้  
...
[โครงสร้างข้อมูลใน DS1307 สามารถอ้างอิงได้จากที่นี่](https://o365cmu-
my.sharepoint.com/:p:/g/personal/arnan_s_cmu_ac_th1/ESSMYgSaHcVIkLTxu29HA5sBWu9lMcxAtVYtv5boG7pxYg?e=veLdhF)
...
### Lab 5 — Reverse Engineer โมดูล LCD Display เพื่อควบคุมบางความสามารถผ่าน
i2c โดยตรง
